...
fn copy_bytecode_from_instance(
    ...
    let bytecode_ptr = u32::from_le_bytes(ret_buffer[4..8].try_into()?);
    let bytecode_len = u32::from_le_bytes(ret_buffer[8..12].try_into()?);
    // 0 is the result discriminator value for the success variant.
    if result != 0 {
        let mut error_bytes = vec![0; bytecode_len as _];
        memory.read(store.as_context(), bytecode_ptr as _, &mut error_bytes)?;
