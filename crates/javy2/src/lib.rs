...
impl JSApiSet for Console {
    fn register<T, U>(&self, context: &JSContextRef, config: &ApiConfig<T, U>) -> Result<()>
    where
        T: Write + 'static,
        U: Write + 'static,
    {
        ...
        let console_log_callback = context.wrap_callback(console_log_to(config.log_stream))?;
        let console_error_callback = context.wrap_callback(console_log_to(config.error_stream))?;
...
pub fn register_apis<T, U>(runtime: &Runtime, config: ApiConfig<T, U>) -> Result<()>
where
    T: Write + 'static,
    U: Write + 'static,
{
