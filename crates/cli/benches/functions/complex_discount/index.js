...
var O = Fr(me);
O._ = O;
const qn = textEncoder.encode(JSON.stringify({
  discountApplicationStrategy: rn.First,
  discounts: []
}));
Lr((n) => {
  let r = JSON.parse(textDecoder.decode(n));
  const t = O.get(r, ["discountNode", "metafield", "value"], "{}");
  if (!O.get(r, ["cart", "buyerIdentity", "customer", "metafield", "value"], "{}"))
    return qn;
  const i = O.chain(r.cart.lines).sortBy((f) => f.quantity).map((f) => En(On({}, f), { id: O.escape(f.id) })).value();
  return O.reduce(i, (f, l) => f + l.quantity, 0) < 0 ? qn : textEncoder.encode(JSON.stringify({
