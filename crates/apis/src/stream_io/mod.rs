...
fn write<'js>(cx: Ctx<'js>, args: &[Value<'js>]) -> Result<usize> {
    let [fd, data, offset, length, ..] = args else {
        bail!("Expected 4 parameters to write to stdout")
    };
    ...
    fn register(&self, runtime: &Runtime, _config: &APIConfig) -> Result<()> {
        runtime.context().with(|cx| {
            ...
            if globals.get::<_, Object>("Javy").is_err() {
                globals.set("Javy", Object::new(cx)?)?
            }

            globals.set("__javy_io_writeSync", Function::new(cx, MutFn::new()))?;
