...
async function main() {
	...
	try {
		childProcess.execFileSync(javyBinaryPath(), getArgs());
	} catch (e) {
...
async function isJavyAvailable() {
	return fs.promises.stat(javyBinaryPath()).then(() => true).catch(() => false)
}
...
async function buildJavy() {
	...
	try {
		...
		childProcess.execSync(`git clone https://github.com/shopify/javy ${repoDir}`);
		...
		console.error("Please make sure you have cmake, Rust with the wasm32-wasi target, wasmtime-cli and cargo-wasi installed");
		console.error("See the javy README for more details.");
	}
	await fs.promises.rename(path.join(repoDir, "target", "release", "javy"), javyBinaryPath());
}
